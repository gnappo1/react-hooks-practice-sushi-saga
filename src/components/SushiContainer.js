...
function SushiContainer({ sushi, handleMoreSushi, amountToSlice, handleEatSushi }) {
  ...
  const renderList = sushi.slice(0, amountToSlice).map(s => <Sushi key={s.id} {...s} handleEatSushi={handleEatSushi }/>)
